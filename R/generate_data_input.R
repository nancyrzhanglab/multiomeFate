...
generate_combn_wave_mat <- function(g, idx_root, num_waves = 20, num_per_wave = 20){
  ...
  mat <- matrix(NA, nrow = ncombn, ncol = num_waves)
  ...
  for(i in 1:length(res$comparison_list)){
    ...
    for(j in 1:ncol(mat)){
      val <- mat[common_row,j]
      percentage <- j/ncol(mat)
      mat[common_row,j] <- round((1-percentage)*val)
      mat[distinct_row_vec,j] <- round(percentage*val)
    }
  }
...
generate_data_input <- function(combn_wave_mat, num_x_per_y = 5,
  ...
  for(i in 1:p2){
    ...
    df_x[idx,"location"] <- round(df_y$location[i]+seq(-spacing/2, spacing/2, length.out = num_x_per_y))
    df_x[idx,"location"] <- pmin(pmax(df_x[idx,"location"], 0), genome_length)
    df_x[idx,"gene"] <- rep(df_y[i,"name"], num_x_per_y)
    df_x[idx,"branch"] <- rep(df_y[i,"branch"], num_x_per_y)
    df_x[idx,"time_lag"] <- round(seq(max_lag, 0, length.out = num_x_per_y))
    df_x[idx,"time_start"] <- round(max(df_y$time_start_scaffold[i] - df_x$time_lag[idx], 0))
    df_x[idx,"time_end"] <- round(max(df_y$time_end_scaffold[i] - df_x$time_lag[idx], time_on))
  }
  ...
  list_ynoise <- lapply(1:num_unrelated_y, function(i){
    vec <- sort(sample(1:genome_length, size = 2*y_unrelated_intervals, replace = F))
  ...
  list_xnoise <- lapply(1:num_unrelated_x, function(i){
    vec <- sort(sample(1:genome_length, size = 2*x_unrelated_intervals, replace = F))
    mat <- matrix(vec, nrow = 2, ncol = length(vec)/2)
    rownames(mat) <- c("time_start", "time_end")

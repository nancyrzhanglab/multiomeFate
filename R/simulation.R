generate_simulation <- function(
  ...
  if(verbose > 0) print("Step 1: Selecting seed cells")
  cluster_idxs <- .kmeans_seed(embedding_mat = embedding_mat,
                               K = K)
  
  if(verbose > 0) print("Step 2: Computing Gaussian distributions")
  ...
  if(verbose > 0) print("Step 3: Computing posterior distributions")
  prob_mat <- .compute_posteriors(
    embedding_mat = embedding_mat,

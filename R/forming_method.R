...
.init_weights <- function(n, form_options){
  if(form_options[["method"]] == "average"){
    return(NA)
  } else if(form_options[["method"]] == "average_weighted"){
    return(rep(1, n))
  }
}
...
.update_estimation_matrices <- function(mat_x, mat_y, mat_x1, mat_y2,
                                        weights,
                                        rec, form_options){
  ...
  if(form_options[["method"]] == "average"){
    ...
    res <- .update_estimation_average_weighted(mat_x, mat_y, mat_x1, mat_y2,
                                               weights, rec, form_options)
  } else {
...
.update_estimation_average <- function(mat_x, mat_y, mat_x1, mat_y2,
  ...
  tmp_x <- t(sapply(1:length(rec), function(i){
    if(form_options$average == "mean"){
      if("dgcMatrix" %in% class(mat_x)){ ## [[note to self: Change this to inherits]]
  ...
  list(mat_x1 = mat_x1, mat_y2 = mat_y2, weights = NA)
}

.update_estimation_average_weighted <- function(mat_x, mat_y, mat_x1, mat_y2,
                                                weights, rec, form_options){
  res <- .update_estimation_average(mat_x, mat_y, mat_x1, mat_y2, 
                             rec, form_options)
  
  # update the weights
  weights <- pmax(weights*form_options$stepsize, form_options$min_weight)
  weights <- c(weights, rep(1, nrow(res$mat_x1) - length(weights)))
  res$weights <- weights

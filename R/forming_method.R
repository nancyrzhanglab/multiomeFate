.init_est_matrices <- function(mat_x, mat_y, vec_start, list_end){
  # initialize
  vec <- c(vec_start, unlist(list_end))
  ...
  mat_x1 <- mat_x[vec,,drop = F]
  mat_y1 <- mat_y[vec,,drop = F]; mat_y2 <- mat_y[vec,,drop = F]
  
  list(mat_x1 = mat_x1, mat_y1 = mat_y1, mat_y2 = mat_y2)
}

.update_estimation_matrices <- function(mat_x1, mat_y1, mat_y2, res, form_options){

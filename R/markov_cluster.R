markov_cluster <- function(count_mat, K, m = 10*K, K0 = 2*K, delta = 0.9,
                           num_restart = 10, max_tries = 500){
  stopifnot(nrow(count_mat) == ncol(count_mat), all(count_mat >= 0),
            inherits(count_mat, c("dgCMatrix", "dgTMatrix")),
            K >= 2, K %% 1 == 0)
  n <- nrow(count_mat)
  ...
  # step 3: optimization
  w_mat <- sapply(1:n, function(i){
    .weighting_optimization(d_mat[i,], vertices)
  })
  w_mat[w_mat < 0] <- 0
  w_mat <- apply(w_mat, 2, function(x){x/sum(x)})
  v_est <- .mult_mat_vec(svd_res$v[,1]*col_sum^(1/2), w_mat)
  v_est <- apply(v_est, 1, function(x){x/sum(x)})
